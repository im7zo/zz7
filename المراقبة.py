...
async def start_watching(event):
    ...
    try:
        ...
        if user.id in watchlist:
            await event.edit(f"المـستـخـدم {username} قيـد المـراقبـة بالفـعل")
            return
        ...
        try:
            ...
            if photos:
                ...
                if new_photo_id != old_data.get('photo'):
                    ...
                    await client.send_file(
                        monitoring_channel,
                        photos[0],  # إرسال الصورة الجديدة مباشرة
                        caption=f" @{watchlist[user_id]} غـير افـتاره",
                        force_document=False
                    )
            ...
            if changes:
                ...
                await client.send_message(monitoring_channel, f" تحـديث فـي حسـاب {user_mention}:\n\n" + "\n".join(changes))
        except Exception as e:
