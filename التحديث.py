...
async def update_all(event):
    ...
    try:
        ...
        # حذف mz.txt القديم إذا موجود
        mz_path = os.path.join(".", "mz.txt")
        ...
        for item in os.listdir(TEMP_EXTRACTED):
            ...
            # إذا الملف هو mz.txt انسخه مباشرة في root
            if item.lower() == "mz.txt":
                dst = os.path.join(".", item)
                shutil.copy2(src, dst)
                continue

            # أما الملفات أو المجلدات الأخرى انسخها داخل Z
